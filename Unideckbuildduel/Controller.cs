    ...
    public class Controller
    {
        ...
        public void StartEverything()
        {
            ...
            if (sd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                if (sd.player1Name != string.Empty) {
                    playerOneName = sd.player1Name;
                }
                if (sd.player2Name != string.Empty)
                {
        ...
        {
            foreach (var building in Game.GetGame.BuildingsForPlayer(num))
            {

                if (building.CardType.Effect== Effect.ProducesOne)
                {
                    if (Game.GetGame.PlayerCardCount(num) < Game.GetGame.PlayerHandSize(num))
                    {
                        Card producedCard = new Card { CardType = building.CardType.EffectCard };
                        Game.GetGame.CardsForPlayer(num).Add(producedCard);
                    }
                }
            }
            ...
            foreach (var card in Game.GetGame.CardsForPlayer(num))
            {
                if (card.CardType.Effect != null)
                { 
                    Effect cardEffect = card.CardType.Effect ?? Effect.noEffect;
                    Game.GetGame.effectsAvailables.Add(cardEffect);
                }
            }
        ...
        public void ExchangeStart(int currentPlayer)
        {
            Window.GetWindow.WriteLine("Player " + PlayerName(currentPlayer) + ", please exchange your cards or discard or finish your turn");
        ...
        public void TurnEnded(int currentPlayer, int turn)
        {
            ...
            if (NumbersOfTurnsToGo>0)
            {
                if (NumbersOfTurnsToGo<=Game.GetGame.Turn)
                {
                    ...
                    {
                        winner = Game.GetGame.PlayerName(0);
                    }
                    else
                    {
                        winner =Game.GetGame.PlayerName(1);
                    }
                    Window.GetWindow.WriteLine("The winner is " + winner + ".");
                }
            }
        ...
        public void enabledDeck()
        {
            Window.GetWindow.btnDeck.Enabled = true;
        }

        public void enabledDrawFromDiscard()
        {
            Window.GetWindow.btnDrawFromDiscard.Enabled = true;
        }
    }
}
